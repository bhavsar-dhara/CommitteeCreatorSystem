...
import main.java.search.userinterface.AuthorProfilePage;
...
public class AuthorProfilePageTest {
	...
	public void testAddButInitialization() {
		...
		assertEquals("page should display addBut when Initialized", page.getButtonArea().getChildren().get(0),
				page.getAddBut());
	}
	...
	public void testRemButInitialization() {
		...
		AuthorProfilePage page = new AuthorProfilePage(reikoForTest, uiForTest);
		assertEquals("page should display remBut when Initialized", page.getButtonArea().getChildren().get(0),
				page.getRemBut());
	}
	...
	public static ArrayList<Author> rFTSimAuth = new ArrayList<Author>(FXCollections.observableArrayList(
			new Author("Yadaiah N.", 7891), new Author("Vikrant Dabas", 285290), new Author("R. Iacono ", 47994802)));
