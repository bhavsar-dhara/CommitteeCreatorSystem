...
public class UIForTest implements UITesting {
	...
	public ObservableList<Author> getSearchResult(Author a) {
		...
		test.setNoOfPublication("20");
		test.setTitle("adlfjlakdsjf");
		return FXCollections.observableArrayList(test);
	}

	@Override
	public ObservableList<Author> getAuthorExperience(Author atr) {
		Author test = new Author("test");
		test.setCommittee("ECOOP");
		test.setRole("Program Chair");
		test.setCheckYear(1998);
		return FXCollections.observableArrayList(test);
	}
