...
/**
 *  This class aim at edit files and rearrange their structure then fill data into database
 *  The table committeeCheck include four things: authorname, checkyear, role, committee)
 *  authorname: a publication's author's name
 *  checkyear: in which year this author ever checked at committee
 *  role: in that year, what kind of role he is
 *  committee: the committee name related this author experience
 *  Not all author include committee info.
 */
public class CommitteeEditor {
    ...
    for (int i = 0; i < listOfFiles.length; i++) {
        if (listOfFiles[i].isFile()) {
            ...
            if(wholeFileName.charAt(0) == '.') continue;
            ...
            try( FileReader fileStream = new FileReader( file );
                ...
                while( (line = bufferedReader.readLine()) != null ) {
                    if (line.length() >= 2)
                        if(line.charAt(1) != ':')   {role = "Member"; name = line;}
            ...
            try {
                //authorName text, checkYear integer, role text, committee text
